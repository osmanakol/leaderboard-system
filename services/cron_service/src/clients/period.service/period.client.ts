...
import { PeriodServiceIdReq, PeriodsReply } from "../../../protos/build/period_service_pb";
...
const makeInActive = (id: string): Promise<Empty> => {
    return new Promise<Empty>((resolve, reject) => {
        client.makeInActive(new PeriodServiceIdReq().setId(id), (err, resp) => {
            if(err)
                return reject(err)
            return resolve(resp)
        })
